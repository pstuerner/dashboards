...
@app.callback(
    ...
    [Input("threshold_slider", "value"), Input("absrel_input", "value")],
)
...
def update_graphs(value):
    ...
    pr_fig.add_trace(
        go.Scatter(
            ...
            x=[-50000, value],
            y=[
                precisions[np.argmax(thresholds >= value)],
                precisions[np.argmax(thresholds >= value)],
            ],
    ...
    pr_fig.add_trace(
        go.Scatter(
            showlegend=False,
            x=[value, value],
            y=[0, precisions[np.argmax(thresholds >= value)]],
            line=dict(color="red", width=1, dash="dash"),
        )
    )
    pr_fig.add_trace(
        go.Scatter(
            ...
            x=[-50000, value],
            y=[
                recalls[np.argmax(thresholds >= value)],
                recalls[np.argmax(thresholds >= value)],
            ],
            line=dict(color="red", width=1, dash="dash"),
        )
    ...
    pr_fig.add_trace(
        go.Scatter(
            showlegend=False,
            x=[value, value],
            y=[0, recalls[np.argmax(thresholds >= value)]],
            line=dict(color="red", width=1, dash="dash"),
        )
    )
