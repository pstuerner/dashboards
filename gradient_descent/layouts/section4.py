...
section4_layout = html.Div([
    dbc.Col([
        ...
        html.P("""
        It's time to compare Batch (BGD), Stochastic (SGD) and Mini-Batch Gradient Descent (MBGD) to get a sense of how they perform in different scenarios. So, let's hit the racetrack 🏇! But before you place a bet on any of the three horses let me first say a few words about the following dashboard.
        """),
        html.P([
            "As in the previous examples, this is a linear regression problem with one dependent variable and one explanatory variable (+ the bias term, hence two parameters to optimize). The underlying data as well as the current regression model for each Gradient Descent step are shown in the left graph. Pay attention: since we are comparing three different algorithms, we get three different results for each step. In the right chart we see the cost function as a contour plot for different $\\theta_0, \\theta_1$ values. The red diamond is the starting point (random initialization) and all three Gradient Descent variants try step by step to reach the green X, which is the global optimum. The dropdown menu below ",
            html.Code('# instances'),
            " controls the size of the data set. To save you from having to perform many individual Gradient Descent steps, you can increase the counter by clicking on ",
            html.Code('NEXT STEP(S)'),
            " and thus perform multiple iterations with a single click. In the table you can set the respective learning schedules and the batch size for Mini-Batch Gradient Descent. To ensure consistent scenarios, the learning schedules, batch size and data set size cannot be changed once the first Gradient Descent step has been calculated. Just click the ",
            html.Code('RESET'),
            " button to start a new experiment and try out new options. Just give it a shot. After the dashboard, I discuss a few noteworthy scenarios that highlight differences and similarities."
        ]),
        ...
        html.Ul([
            ...
            html.Li([
                ...
                html.P([
                    ...
                    ', the algorithm is almost identical to Batch Gradient Descent. It holds that smaller batch sizes push the algorithm towards Stochastic Gradient Descent and larger batch sizes push it towards Batch Gradient Descent. In the extreme cases, i.e. ',
                    html.Code('batch_size=1'),
                    ' and ',
                    html.Code('batch_size=m'),
                    ', Mini-batch Gradient Descent is equal to Stochastic and Batch Gradient Descent, respectively.'
                ])
            ]),
        ]),
    ], xs=12, sm=12, md=12, lg=8, className='bs-component center'), 
