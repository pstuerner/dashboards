...
section3_layout = html.Div([
    dbc.Col([
        ...
        """),
        html.P([
            "Cumbersome 🤢! However, this is not a problem, because it is not necessary to determine the gradient by individual evaluation of the partial derivatives. It is far more convenient to compute a so-called gradient vector $\\nabla_{{\\theta}}\\textrm{MSE}(\\theta)$ in order to come up with the entire gradient in one go. The determination of the gradient vector is more complicated and outside the scope of this post. In case you are curious about it, I recommend this answer on ",
